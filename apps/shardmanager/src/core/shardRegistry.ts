...
export class ShardRegistry {
  ...
  public register(
    ...
    logger.info(
      `Registered process ${wsId} with shards [${shardIds.join(", ")}]`,
    );
  }
   ...
   */
  public getProcessByShardId(
    shardId: number,
  ): (Omit<ShardProcessInfo, "wsConnection"> & { wsId: string }) | null {
    ...
    this.heartbeatTimer = setInterval(() => {
      ...
      for (const [wsId, process] of this.processes.entries()) {
        if (now - process.lastHeartbeat > HEARTBEAT_TIMEOUT_MS) {
          logger.warn(
            `Process ${wsId} heartbeat timeout (shards: [${process.shardIds.join(", ")}])`,
          );
