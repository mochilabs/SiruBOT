  ...
  return (
    <div className="group">
      <div className="track-row flex items-center gap-4 px-3 py-3 sm:px-4">
        <div
          className={`rank-badge h-8 w-8 shrink-0 text-sm font-mono ${isTop3 ? "top-3" : ""}`}
        >
          <span className={isTop3 ? "" : "text-[var(--color-text-muted)]"}>
            {rank}
          </span>
        </div>
      ...
      {!isLast && (
        <Separator.Root className="mx-3 h-px bg-[var(--color-border)] sm:mx-4" />
      )}
    </div>
  );
}
    ...
    <div className="glass-card flex items-center gap-3 p-3">
      {track.thumbnail ? (
        <Image
          ...
          width={48}
          height={48}
          className="rounded-lg object-cover ring-1 ring-[var(--color-border)]"
        />
      ) : (
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="text-xs text-[var(--color-text-muted)]">
          {track.totalPlays.toLocaleString()}회 재생
        </p>
      </div>
    </div>
  );
