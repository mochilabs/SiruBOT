		...
		<div className="glass-panel p-6 space-y-6 transition-all duration-300 hover:border-primary/30">
			<div className="flex items-center justify-between gap-4">
				<div className="flex items-center gap-3">
					...
					<div>
						<h3 className="text-lg font-black tracking-tighter text-foreground">클러스터 #{index + 1}</h3>
			...
			<div className="grid grid-cols-2 gap-4">
				{[
					{ label: "길드", value: process.guilds.toLocaleString(), icon: Server },
					{ label: "플레이어 개수", value: process.players.toLocaleString(), icon: RadioTower },
					{ label: "메모리", value: formatMemory(process.memoryUsage), icon: HardDrive },
					{ label: "업타임", value: formatUptime(process.uptime), icon: Clock3 },
				].map((item, i) => (
					<div key={i} className="glass-overlay rounded-xl p-4 space-y-2 group hover:bg-white/10 transition-colors">
			...
			<div className="pt-2 flex items-center justify-between text-[13px] font-medium text-muted-foreground/40 border-t border-white/5">
				<div className="flex items-center gap-2">
					<span className="w-2 h-2 rounded-full bg-primary/40 animate-pulse" />
					마지막 하트비트: {formatRelativeTime(process.lastHeartbeat)}
				</div>
			</div>
		</div>
	);
